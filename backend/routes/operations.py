...
PDF_COLUMN_DEFINITIONS = {
    ...
    # ── Phase 23: Smart PDF Configuration types (operational documents) ──
    'lkp': [
        {'key': 'lkp_number', 'label': 'No. LKP', 'required': True},
    ...
    'payslip': [
        {'key': 'employee_name', 'label': 'Nama Karyawan', 'required': True},
        {'key': 'employee_code', 'label': 'Kode Karyawan'},
        {'key': 'period', 'label': 'Periode', 'required': True},
        {'key': 'pay_scheme', 'label': 'Skema Gaji'},
        {'key': 'days_hadir', 'label': 'Hari Hadir'},
        {'key': 'hours_worked', 'label': 'Jam Kerja'},
        {'key': 'overtime_hours', 'label': 'Jam Lembur'},
        {'key': 'gross_pay', 'label': 'Total Pendapatan'},
        {'key': 'total_deductions', 'label': 'Total Potongan'},
        {'key': 'net_pay', 'label': 'Gaji Bersih', 'required': True},
        {'key': 'nik', 'label': 'NIK'},
        {'key': 'npwp', 'label': 'NPWP'},
        {'key': 'department', 'label': 'Departemen'},
        {'key': 'position', 'label': 'Jabatan'},
        {'key': 'tenure', 'label': 'Masa Kerja'},
    ],
    'shift-report': [
        ...
        {'key': 'handover_to', 'label': 'Diserahkan ke'},
    ],
}
...
async def create_pdf_export_config(request: Request):
    """Create a new PDF export config (extended schema for Smart PDF Config Phase 23)."""
    user = await require_auth(request)
    db = get_db()
    body = await request.json()
    ...
    # For advanced types (lkp/payslip/shift-report), columns are optional;
    # other types still require at least 1 column.
    if not columns and pdf_type not in ('lkp', 'payslip', 'shift-report'):
    ...
    doc = {
        ...
        # Phase 23 advanced fields (optional)
    ...
    # Drop None values to keep schema clean (resolver will fill defaults)
...
async def update_pdf_export_config(config_id: str, request: Request):
    ...
    # Phase 23 advanced fields — direct passthrough (None means "unset")
    for fld in ('sections', 'header', 'footer', 'branding', 'page', 'typography',
                'format', 'language', 'custom_labels'):
        if fld in body:
            update[fld] = body[fld]
