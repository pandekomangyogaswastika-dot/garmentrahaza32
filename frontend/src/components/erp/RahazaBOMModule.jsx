...
/* ─── PT Rahaza · Fase 5b — BOM KG per Model + Size ───────────────────────────
   ...
   - 2026-05-07: REPLACED free-text material input with dropdown from master data
     for data integrity (audit fix C4). Supports + Tambah Material Baru on-the-fly.
 ───────────────────────────────────────────────────────────────────────────── */
...
export default function RahazaBOMModule({ token }) {
  ...
  // Master data for material dropdowns
  const [allMaterials, setAllMaterials] = useState([]);  // all rahaza_materials (active)
  ...
  const [addMatModal, setAddMatModal] = useState(null);  // { context: 'yarn'|'accessory', rowIdx, form }

  const headers = { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' };

  const loadMaterials = useCallback(async () => {
    const r = await fetch('/api/rahaza/materials', { headers: { Authorization: `Bearer ${token}` } });
    if (r.ok) {
      const data = await r.json();
      setAllMaterials((data || []).filter(m => m.active));
    }
  }, [token]);
  ...
  useEffect(() => { loadBase(); loadMaterials(); }, [loadBase, loadMaterials]);
  useEffect(() => { loadMatrix(); }, [loadMatrix]);

  // Yarn-type materials (filter)
  const yarnMaterials = allMaterials.filter(m => m.type === 'yarn');
  const accessoryMaterials = allMaterials.filter(m => m.type === 'accessory');

  const openEditor = async (sizeId, existingBomId) => {
    let form = {
      yarn_materials: [{ material_id: '', name: '', code: '', yarn_type: '', unit: 'kg', qty_kg: '', notes: '' }],
      accessory_materials: [],
      notes: '',
    };
    if (existingBomId) {
      ...
      if (r.ok) {
        ...
        form = {
          yarn_materials:      (b.yarn_materials || []).map(y => ({
            material_id: y.material_id || '',
            name: y.name || '',
            code: y.code || '',
            yarn_type: y.yarn_type || '',
            unit: y.unit || 'kg',
            qty_kg: String(y.qty_kg ?? ''),
            notes: y.notes || '',
          })),
          accessory_materials: (b.accessory_materials || []).map(a => ({
        ...
        if (form.yarn_materials.length === 0) form.yarn_materials = [{ material_id: '', name:'', code:'', yarn_type:'', unit:'kg', qty_kg:'', notes:'' }];
      }
    }
    setFormError('');
  ...
  const selectYarnMaterial = (idx, materialId) => {
    if (materialId === '__add__') {
      setAddMatModal({ context: 'yarn', rowIdx: idx, form: { name: '', code: '', yarn_type: '', unit: 'kg' } });
      return;
    }
    const m = allMaterials.find(x => x.id === materialId);
    if (!m) {
      // Cleared
      updateYarn(idx, 'material_id', '');
      updateYarn(idx, 'name', '');
      updateYarn(idx, 'code', '');
      return;
    }
    setEditor(e => ({
      ...
      form: {
        ...
        yarn_materials: e.form.yarn_materials.map((y, i) => i === idx ? {
          ...
          name: m.name,
          code: m.code,
          yarn_type: m.description || y.yarn_type || '',
          unit: m.unit || 'kg',
        } : y),
      },
    }));
  ...
  const addYarn = () => setEditor(e => ({
    ...e, form: { ...e.form, yarn_materials: [...e.form.yarn_materials, { material_id: '', name:'', code:'', yarn_type:'', unit:'kg', qty_kg:'', notes:'' }] }
  }));
  const removeYarn = (idx) => setEditor(e => ({
    ...e, form: { ...e.form, yarn_materials: e.form.yarn_materials.filter((_, i) => i !== idx) }
  }));

  // ── Accessory row mutations ────────────────────────────────────────────────
  ...
  const selectAccMaterial = (idx, materialId) => {
    if (materialId === '__add__') {
      setAddMatModal({ context: 'accessory', rowIdx: idx, form: { name: '', code: '', unit: 'pcs' } });
      return;
    }
    const m = allMaterials.find(x => x.id === materialId);
    if (!m) {
      updateAcc(idx, 'material_id', '');
      updateAcc(idx, 'name', '');
      updateAcc(idx, 'code', '');
      return;
    }
  ...
  const addAcc = () => setEditor(e => ({
    ...e, form: { ...e.form, accessory_materials: [...e.form.accessory_materials, { material_id: '', name:'', code:'', qty:'', unit:'pcs', notes:'' }] }
  }));
  ...
  const submitAddMaterial = async () => {
    ...
    const { context, rowIdx, form } = addMatModal;
    if (!form.name?.trim()) {
      setAddMatModal(m => ({ ...m, error: 'Nama material wajib diisi.' }));
      return;
    }
    try {
      const r = await fetch('/api/rahaza/materials/quick-add', {
        method: 'POST', headers, body: JSON.stringify({
          ...
          type: context === 'yarn' ? 'yarn' : 'accessory',
          unit: form.unit || (context === 'yarn' ? 'kg' : 'pcs'),
        }),
      });
      if (!r.ok) {
        const errBody = await r.json().catch(() => ({}));
        throw new Error(errBody.detail || `Gagal menambah material (HTTP ${r.status})`);
      }
      const newMat = await r.json();
      // Refresh master list
      const updatedList = [...allMaterials, newMat];
      ...
      if (context === 'yarn') {
        setEditor(e => ({
          ...
          form: {
            ...e.form,
            yarn_materials: e.form.yarn_materials.map((y, i) => i === rowIdx ? {
              ...y,
        ...
        setEditor(e => ({
          ...
          form: {
            ...e.form,
            accessory_materials: e.form.accessory_materials.map((a, i) => i === rowIdx ? {
              ...a,
              material_id: newMat.id,
              name: newMat.name,
              code: newMat.code,
              unit: newMat.unit || 'pcs',
            } : a),
          },
        }));
      }
      ...
      setAddMatModal(m => ({ ...m, error: err.message }));
    }
  };
  ...
  const saveBOM = async () => {
    ...
    try {
      const yarns = editor.form.yarn_materials
        .filter(y => y.material_id && y.name && Number(y.qty_kg) > 0)
        .map(y => ({
          material_id: y.material_id,
          name: y.name,
          code: y.code,
          yarn_type: y.yarn_type || '',
          unit: y.unit || 'kg',
          qty_kg: Number(y.qty_kg),
          notes: y.notes || '',
        }));
      const accs = editor.form.accessory_materials
        .filter(a => a.material_id && a.name && Number(a.qty) > 0)
        .map(a => ({
          material_id: a.material_id,
          name: a.name,
          code: a.code,
      ...
      // Detect rows with qty but no material_id (user error)
      const orphanYarns = editor.form.yarn_materials.filter(y => !y.material_id && (y.name || Number(y.qty_kg) > 0));
      const orphanAccs = editor.form.accessory_materials.filter(a => !a.material_id && (a.name || Number(a.qty) > 0));
      if (orphanYarns.length || orphanAccs.length) {
        throw new Error('Setiap baris harus memilih material dari master (gunakan tombol "+ Tambah Material Baru" jika belum ada).');
      }
    ...
    <div className="space-y-5" data-testid="rahaza-bom-page">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Kebutuhan benang (KG) & aksesoris per Model dan Size. Material dipilih dari master untuk konsistensi data.</p>
        </div>
      ...
      {editor && (
        <Modal onClose={() => setEditor(null)} title={`BOM ${selectedModel?.code} · Size ${sizes.find(s => s.id === editor.size_id)?.code || ''}`} size="xl">
          <div className="space-y-5" data-testid="bom-editor-form">
            ...
            <div className="bg-blue-400/10 border border-blue-300/20 rounded-lg p-3 text-xs text-foreground/70">
              💡 Pilih material dari dropdown master. Jika belum ada, klik <b>+ Tambah Material Baru</b> untuk menambahkan langsung.
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={addYarn} className="text-xs text-primary hover:text-primary/80 flex items-center gap-1" data-testid="bom-add-yarn-btn"><Plus className="w-3 h-3" /> Tambah Baris Benang</button>
              </div>
              <GlassPanel className="p-0 overflow-hidden">
                <table className="w-full text-sm">
                  <thead className="bg-[var(--glass-bg)]">
                    <tr className="text-left text-xs text-muted-foreground">
                      <th className="px-3 py-2 w-[40%]">Material (Master Data)</th>
                      <th className="px-3 py-2">Jenis / Komposisi</th>
                      <th className="px-3 py-2 w-28">Qty (KG/pcs)</th>
                  ...
                  <tbody>
                    {editor.form.yarn_materials.map((y, idx) => (
                      <tr key={idx} className="border-t border-[var(--glass-border)]">
                        <td className="px-2 py-1">
                          <select
                            value={y.material_id || ''}
                            onChange={e => selectYarnMaterial(idx, e.target.value)}
                            className="w-full h-9 px-2 rounded-lg border border-[var(--glass-border)] bg-[var(--input-surface)] text-sm text-foreground"
                            data-testid={`bom-yarn-${idx}-material`}
                          >
                            <option value="">— Pilih Benang —</option>
                            {yarnMaterials.map(m => (
                              <option key={m.id} value={m.id}>{m.code} · {m.name}</option>
                            ))}
                            <option value="__add__">+ Tambah Material Baru…</option>
                          </select>
                        </td>
                        <td className="px-2 py-1">
                          <GlassInput value={y.yarn_type} onChange={e => updateYarn(idx, 'yarn_type', e.target.value)} placeholder="Acrylic / 100%" />
                        </td>
                        <td className="px-2 py-1">
                          <GlassInput type="number" step="0.001" value={y.qty_kg} onChange={e => updateYarn(idx, 'qty_kg', e.target.value)} placeholder="0.300" data-testid={`bom-yarn-${idx}-qty`} />
                        </td>
                        <td className="px-2 py-1">
                          <GlassInput value={y.notes} onChange={e => updateYarn(idx, 'notes', e.target.value)} placeholder="warna utama" />
                        </td>
                    ...
                    {editor.form.yarn_materials.length === 0 && (
                      <tr><td colSpan={5} className="text-center py-4 text-xs text-muted-foreground">Belum ada benang.</td></tr>
                    )}
                  </tbody>
                </table>
              </GlassPanel>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={addAcc} className="text-xs text-primary hover:text-primary/80 flex items-center gap-1" data-testid="bom-add-accessory-btn"><Plus className="w-3 h-3" /> Tambah Baris Aksesoris</button>
              </div>
              <GlassPanel className="p-0 overflow-hidden">
                <table className="w-full text-sm">
                  <thead className="bg-[var(--glass-bg)]">
                  ...
                  <tbody>
                    {editor.form.accessory_materials.map((a, idx) => (
                      <tr key={idx} className="border-t border-[var(--glass-border)]">
                        <td className="px-2 py-1">
                          <select
                            value={a.material_id || ''}
                            onChange={e => selectAccMaterial(idx, e.target.value)}
                            className="w-full h-9 px-2 rounded-lg border border-[var(--glass-border)] bg-[var(--input-surface)] text-sm text-foreground"
                            data-testid={`bom-acc-${idx}-material`}
                          >
                        ...
                        <td className="px-2 py-1">
                          <input type="text" value={a.unit || 'pcs'} disabled
                            className="w-full h-9 px-2 rounded-lg border border-[var(--glass-border)] bg-[var(--input-surface)] text-sm text-foreground/60" />
                        </td>
                    ...
                    {editor.form.accessory_materials.length === 0 && (
                      <tr><td colSpan={5} className="text-center py-4 text-xs text-muted-foreground">Belum ada aksesoris. (Opsional)</td></tr>
                    )}
                  </tbody>
                </table>
              </GlassPanel>
      ...
      {/* Quick add material modal */}
      {addMatModal && (
        <Modal onClose={() => setAddMatModal(null)} title={`Tambah Material Baru (${addMatModal.context === 'yarn' ? 'Benang' : 'Aksesoris'})`} size="sm">
          <div className="space-y-3" data-testid="bom-quickadd-material">
            {addMatModal.error && <div className="bg-red-400/10 border border-red-300/20 rounded-lg p-3 text-sm text-red-300">{addMatModal.error}</div>}
            <div>
              <label className="block text-xs font-medium text-foreground/70 mb-1">Nama Material<span className="text-red-400 ml-0.5">*</span></label>
              <GlassInput
                value={addMatModal.form.name}
                onChange={e => setAddMatModal(m => ({ ...m, form: { ...m.form, name: e.target.value } }))}
                placeholder={addMatModal.context === 'yarn' ? 'cth: Benang Acrylic 2/28' : 'cth: Kancing Plastik 18mm'}
                data-testid="quickadd-material-name"
              />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-foreground/70 mb-1">Kode (opsional)</label>
                <GlassInput
                  value={addMatModal.form.code}
                  onChange={e => setAddMatModal(m => ({ ...m, form: { ...m.form, code: e.target.value } }))}
              ...
              <div>
                ...
                <select
                  value={addMatModal.form.unit}
                  onChange={e => setAddMatModal(m => ({ ...m, form: { ...m.form, unit: e.target.value } }))}
                  className="w-full h-9 px-2 rounded-lg border border-[var(--glass-border)] bg-[var(--input-surface)] text-sm text-foreground">
                  {addMatModal.context === 'yarn' ? (
                    <>
            ...
            {addMatModal.context === 'yarn' && (
              <div>
                <label className="block text-xs font-medium text-foreground/70 mb-1">Jenis / Komposisi (opsional)</label>
                <GlassInput
                  value={addMatModal.form.yarn_type || ''}
                  onChange={e => setAddMatModal(m => ({ ...m, form: { ...m.form, yarn_type: e.target.value } }))}
                  placeholder="cth: Acrylic 100%"
                />
              </div>
            )}
            <div className="flex items-center justify-end gap-2 pt-2">
              <Button variant="ghost" onClick={() => setAddMatModal(null)}>Batal</Button>
              <Button onClick={submitAddMaterial} data-testid="quickadd-material-submit">Tambah & Pilih</Button>
            </div>
          </div>
        </Modal>
      )}
