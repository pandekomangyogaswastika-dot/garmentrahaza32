...
export function DataTable({
  ...
  const filtered = useMemo(() => {
    ...
    Object.entries(filterState).forEach(([k, v]) => {
      ...
      if (!f) return;
      if (f.type === 'select') {
        // Special value: 'not_completed' → exclude completed/cancelled
        if (v === 'not_completed') {
          const DONE = new Set(['completed', 'cancelled', 'done', 'archived', 'rejected', 'closed']);
          out = out.filter(r => {
            const cell = f.accessor ? f.accessor(r) : r[k];
            return !DONE.has(String(cell));
          });
        } else {
          out = out.filter(r => {
            const cell = f.accessor ? f.accessor(r) : r[k];
