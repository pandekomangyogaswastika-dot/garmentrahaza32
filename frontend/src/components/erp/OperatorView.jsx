            ...
            {quick.row.is_qc ? (
              ...
              <div>
                <label className="block text-xs font-medium text-foreground/70 mb-1">
                  Qty Output{quick.bundle ? <span className="text-[10px] text-muted-foreground ml-1">max {quick.bundle.qty_remaining}</span> : null}
                </label>
                <LusinPcsInput
                  value={Number(quick.qty) || 0}
                  onChange={(totalPcs) => setQuick(q => ({ ...q, qty: String(totalPcs) }))}
                  testId="op-qty-input"
                  max={quick.bundle ? Number(quick.bundle.qty_remaining) : undefined}
                />
